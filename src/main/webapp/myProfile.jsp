...
<fmt:setBundle basename="com.globalapps.web.common.resources.Resources" />
		...
		<form action="/globalapp/updateAvatar.do" id="ProfileForm" method="post" enctype="multipart/form-data">
			<label for="nuevo_avatar" style="display: flex;">
				...
				<div class="myProfile-avatarContainer" style="background-image: url(<c:choose><c:when test="${not empty userSessionInfo.credential.avatar}">/globalapp/getAvatar.do</c:when><c:otherwise>images/userAvatar.png</c:otherwise></c:choose>);
